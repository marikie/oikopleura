...
class Alignment():
    ...
    def fromMAFEntry(cls, mafEntry):
        ...
        cigar = ''.join(cigarParts(rStart, iter(alignmentColums),
                                   int(queryLine[5])-rEnd-1))

        return cls(score=score, sense=sense, mismap=mismap, don=don, acc=acc,
                   cigar=cigar, gChr=gChr, gStart=gStart, gEnd=gEnd,
                   gStrand=gStrand, gLength=gLength, gSeq=gSeq, rID=rID,
                   rStart=rStart, rEnd=rEnd, rLength=rLength, rStrand=rStrand,
                   rSeq=rSeq)
    ...
    def _MAF(self):
        ...
        maxLength_start = len(max([str(self.gStart), str(self.rStart)],
                                  key=len))
        maxLength_end = len(max([str(self.gEnd), str(self.rEnd)], key=len))
        maxLength_length = len(max([str(self.gLength), str(self.rLength)],
                                   key=len))
