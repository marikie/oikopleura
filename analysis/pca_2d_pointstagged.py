...
# 散布図の描画
scatter = plt.scatter(X_pca[:, 0], X_pca[:, 1], c=cluster_labels, cmap='viridis', alpha=0.7, s=70)
plt.scatter(centroids[:, 0], centroids[:, 1], marker='X', s=200, c='red', label='Center of the Cluster')

# 各データ点にファイル名のラベルを追加
for i, file_name in enumerate(file_names):
    # ファイル名から不要な部分を削除（短くするため）
    short_name = file_name.split('_')[0]  # 最初の6文字だけ表示
...
plt.grid(True)
plt.savefig('pca_clustering_result_with_labels.png', dpi=300, bbox_inches='tight')
plt.tight_layout()
plt.show()
