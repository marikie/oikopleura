...
generate_plot<-function(file_path, filename=0, ymax_plus, orgName, deci){
  ...
  pct_yaxs_max <- round(max(na.omit(as.numeric(conv.data.norm[conv.label.all.sorted]))), deci)+ymax_plus
  # 2 decimal place
  #pct_yaxs_max <- round(max(na.omit(as.numeric(conv.data.norm[conv.label.all.sorted]))), 2)+ymax_plus
  print("pct_yaxs_max:")
  print(pct_yaxs_max)
  ...
  if(!filename==0){
    ...
    par(family="mono", mar=c(7.5, 10, 2, 0.5), cex.axis=2) # Increase the size of y-axis numbers # Increase the bottom, left, top, and right margins
    ...
    # Ensure the y-axis limit is set correctly
    if(deci==1){
        # max is 1 decimal place
        axis(2, at = seq(0, pct_yaxs_max, by = 0.05), las = 1)
    }
    if(deci==2){
        # max is 2 decimal place
        axis(2, at = seq(0, pct_yaxs_max, by = 0.005), las = 1)
    }
    ...
    if(deci==1){
        mtext("#Subs/#OrigTrinucs (%)", side=2, line=6, cex=2.5) # cexでサイズ調整
    }
    if(deci==2){
        mtext("#Subs/#OrigTrinucs (%)", side=2, line=8, cex=2.5) # cexでサイズ調整
    }
...
generate_plot(tsv_path, filename = graph_path, as.numeric(ymax_plus), orgName, as.numeric(decimal))