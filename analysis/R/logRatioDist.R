...
generate_plots <- function(tsv_path) {
  ...
  p <- ggplot(data, aes(x = trans, y = logRatio_ncds)) +
    ...
    geom_text(aes(label = oriType), hjust = -0.2, vjust = 0.5, size = 3) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
    geom_hline(yintercept = mean_val, linetype = "solid", color = "blue", size = 1) +
    geom_hline(yintercept = mean_val + sd_val, linetype = "dotted", color = "green", size = 1) +
    geom_hline(yintercept = mean_val - sd_val, linetype = "dotted", color = "green", size = 1) +
    geom_hline(yintercept = mean_val + 2 * sd_val, linetype = "dotted", color = "orange", size = 1) +
    geom_hline(yintercept = mean_val - 2 * sd_val, linetype = "dotted", color = "orange", size = 1) +
    labs(
  ...
  p2 <- ggplot(data, aes(x = "All Data", y = logRatio_ncds)) +
    ...
    geom_text(aes(label = mutType), hjust = -0.2, vjust = 0.5, size = 3, angle = 45) +
