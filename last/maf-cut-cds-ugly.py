...
def shrunkRange(beg, end):  # try to shrink the range, but ensure length >= 1
    b = min(beg + 1, end - 1)  # try to increase begin coordinate by 1
    e = max(end - 1, b + 1)    # try to decrease end coordinate by 1
    return b, e
