...
def alignmentInput(lines):
    ...
    for line in lines:
        ...
        if line[0] == "#":
            ...
            strand = w[8]
            if len(strand) > 1:
                raise RuntimeError("can't handle translated PSL format")
            chr2, seqlen2 = w[9], int(w[10])
            chr1, seqlen1 = w[13], int(w[14])
            sizes = commaSeparatedInts(w[18])
            beg2s = commaSeparatedInts(w[19])
            beg1s = commaSeparatedInts(w[20])
            if strand == "-":
                beg2s = (i - seqlen2 for i in beg2s)
...
def alignmentSortData(alignments, seqIndex, otherNamesToRanksAndFlips):
    ...
    for i in alignments:
        ...
        otherPos = otherFlip * abs(
            blocks[0][otherIndex] + blocks[-1][otherIndex] + blocks[-1][2]
        )
        numOfAlignedLetterPairs = sum(i[2] for i in blocks)
...
def mySortedRanges(seqRanges, sortOpt, seqIndex, alignments, otherRanges):
    ...
    if sortOpt == "3":
        ...
        alns = sorted(
            alignmentSortData(alignments, seqIndex, otherNamesToRanksAndFlips)
        )
...
def allSortedRanges(
    opts, alignments, alignmentsB, seqRanges1, seqRangesB1, seqRanges2, seqRangesB2
):
...
def alignmentPixels(width, height, alignments, bp_per_pix, rangeDict1, rangeDict2):
    ...
    for seq1, seq2, blocks in alignments:
        ...
        for beg1, beg2, size in blocks:
            ...
            if isReverse1 == isReverse2:
                drawLineForward(hits, width, bp_per_pix, ori1 + beg1, ori2 + beg2, size)
            else:
                drawLineReverse(
                    hits, width, bp_per_pix, ori1 + beg1, ori2 - beg2 - 1, size
                )
    return hits
...
def annotsFromGpd(opts, rangeDict, fields, geneName):  # xxx split on tabs?
    ...
    if seqName in rangeDict:
        ...
        for beg, end in zip(exonBegs, exonEnds):
            ...
            e = min(end, cdsEnd)
            if b < e:
                yield 400, opts.cds_color, seqName, b, e, ""
    ...
    for fileName in fileNames:
        for line in myOpen(fileName):
            ...
            if n > 10 and w[8] in "+C-" and isDig(w[5]) and isDig(w[6]):
                ...
                if seq not in rangeDict:
                    continue  # do this ASAP for speed
                beg = int(w[5]) - 1
                end = int(w[6])
...
def axisImage(
    labels, rangePixBegs, rangePixLens, textRot, textAln, font, image_mode, opts
):
...
def getFont(opts):
    ...
    try:
        ...
        p = subprocess.Popen(
            x, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
        )
...
def lastDotplot(opts, args):
    ...
    if not alignments:
        raise RuntimeError("there are no alignments")
    ...
    cutRanges1 = list(
        trimmed(seqRanges1, coverDict1, minAlignedBases, maxGap1, pad, pad)
    )
    cutRanges2 = list(
        trimmed(seqRanges2, coverDict2, minAlignedBases, maxGap2, pad, pad)
    )
    ...
    cutRangesB1 = trimmed(seqRangesB1, coverDictB1, minAlignedBases, maxGapB1, 0, 0)
    cutRangesB2 = trimmed(seqRangesB2, coverDictB2, minAlignedBases, maxGapB2, 0, 0)

    logging.info("sorting...")
    sortOut = allSortedRanges(
        opts, alignments, alignmentsB, cutRanges1, cutRangesB1, cutRanges2, cutRangesB2
    )
    ...
    annots1 = readAnnots(opts, font, textDraw, sortedRanges1, opts.height, opts.bed1)
    bMargin = annots1[-1]

    annots2 = readAnnots(opts, font, textDraw, sortedRanges2, opts.width, opts.bed2)
    rMargin = annots2[-1]
    ...
    rangeDict1 = dict(
        rangesAndOriginsPerSeq(sortedRanges1, rangePixBegs1, rangePixLens1, bpPerPix)
    )
    ...
    rangeDict2 = dict(
        rangesAndOriginsPerSeq(sortedRanges2, rangePixBegs2, rangePixLens2, bpPerPix)
    )
    ...
    hits = alignmentPixels(
        width, height, allAlignments, bpPerPix, rangeDict1, rangeDict2
    )
    ...
    for i in range(height):
        for j in range(width):
            ...
            xy = j, i
            if store_value == 1:
                im.putpixel(xy, forward_color)
            elif store_value == 2:
                im.putpixel(xy, reverse_color)
            elif store_value == 3:
                im.putpixel(xy, overlap_color)
    ...
    if opts.fontsize != 0:
        ...
        annoImage1 = annoTextImage(
            opts, image_mode, font, bMargin, width, boxes1, False
        )
        annoImage1 = annoImage1.transpose(Image.ROTATE_90)
        annoImage2 = annoTextImage(
            opts, image_mode, font, rMargin, height, boxes2, True
        )
...
if __name__ == "__main__":
    usage = """%prog --help
   or: %prog [options] maf-or-psl-or-tab-alignments dotplot.png
   or: %prog [options] maf-or-psl-or-tab-alignments dotplot.gif
   or: ..."""
    ...
    op.add_option(
        ...
        default=100,
        metavar="M",
        help="maximum number of horizontal or vertical sequences " "(default=%default)",
    )
    op.add_option(
        "-1",
        "--seq1",
    ...
    op.add_option(
        ...
        default="0",
        metavar="N",
        help="genome1 sequence orientation: 0=forward orientation, "
    ...
    op.add_option(
        ...
        default="0",
        metavar="N",
        help="genome2 sequence orientation: 0=forward orientation, "
        "1=alignment orientation (default=%default)",
    )
    op.add_option(
        "--max-gap1",
        metavar="FRAC",
        default="0.5,2",
        help="maximum unaligned (end,mid) gap in genome1: "
    ...
    op.add_option(
        ...
        metavar="FRAC",
        default="0.5,2",
        help="maximum unaligned (end,mid) gap in genome2: "
        "fraction of aligned length (default=%default)",
    )
    op.add_option(
        "--pad",
        ...
        type="float",
        default=0.04,
        help="pad length when cutting unaligned gaps: "
        "fraction of aligned length (default=%default)",
    )
    op.add_option(
        "-j",
        ...
        default="0",
        metavar="N",
        help="join: 0=nothing, 1=alignments adjacent in genome1, "
    ...
    og.add_option(
        "-f", "--fontfile", metavar="FILE", help="TrueType or OpenType font file"
    )
    og.add_option(
        "-s",
        "--fontsize",
    ...
    og.add_option(
        ...
        default=0,
        metavar="N",
        help="genome1 labels: 0=name, 1=name:length, "
    ...
    og.add_option(
        ...
        default=0,
        metavar="N",
        help="genome2 labels: 0=name, 1=name:length, "
        "2=name:start:length, 3=name:start-end (default=%default)",
    )
    og.add_option(
        "--rot1",
    ...
    og.add_option(
        "--margin-color", metavar="COLOR", default="#dcdcdc", help="margin color"
    )
    og.add_option(
        "--exon-color",
        metavar="COLOR",
